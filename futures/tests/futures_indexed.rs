...
use futures::stream::futures_indexed;
use futures::prelude::*;
...
fn works_1() {
    ...
    let stream = futures_indexed(vec![a_rx, b_rx, c_rx]);
...
fn works_2() {
    ...
    let stream = futures_indexed(vec![
...
fn from_iterator() {
    ...
    use futures::stream::FuturesIndexed;

    let stream = vec![
        ok::<u32, ()>(1),
        ok::<u32, ()>(2),
        ok::<u32, ()>(3)
    ].into_iter().collect::<FuturesIndexed<_>>();
